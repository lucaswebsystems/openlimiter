...
async fn collect_account_guarded<T: Transport>(
    ...
    let _lease = match policy.begin(DetectedProviderId::Kimi, &account_id, now_ms) {
        Ok(lease) => lease,
    ...
    let abort_provider = match &outcome {
        KimiOutcome::Fallback {
            reason: KimiFailure::ProviderBlocked,
            ..
        } => {
            policy.block_provider(DetectedProviderId::Kimi, now_ms, BLOCKED_BACKOFF_SECONDS);
            true
        }
        KimiOutcome::Fallback {
            reason: KimiFailure::RateLimited,
...
fn pass_read_succeeded(outcome: &KimiOutcome) -> bool {
    matches!(
        outcome,
        KimiOutcome::CacheCommitted { .. } | KimiOutcome::Cached { .. }
    )
}
...
mod tests {
    ...
    fn home_refresh_reports_a_committed_read_and_a_login_failure() {
        assert!(pass_read_succeeded(&KimiOutcome::CacheCommitted {
            account_id: "fixture".into()
        }));
        assert!(pass_read_succeeded(&KimiOutcome::Cached {
            account_id: "fixture".into(),
            retry_at: "2026-09-08T12:00:00Z".into()
        }));
        assert!(!pass_read_succeeded(&KimiOutcome::ReopenCli {
            account_id: "fixture".into(),
