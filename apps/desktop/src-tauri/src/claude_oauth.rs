...
fn pass_read_succeeded(outcome: &ClaudeOauthOutcome) -> bool {
    matches!(
        outcome,
        ClaudeOauthOutcome::CacheCommitted { .. } | ClaudeOauthOutcome::Cached { .. }
    )
}
...
mod tests {
    ...
    fn home_refresh_reports_a_committed_read_and_a_login_failure() {
        ...
        assert!(pass_read_succeeded(&ClaudeOauthOutcome::Cached {
            account_id: "fixture".into(),
            retry_at: "2026-09-08T12:00:00Z".into()
        }));
        assert!(!pass_read_succeeded(&ClaudeOauthOutcome::ReopenCli {
            account_id: "fixture".into(),
