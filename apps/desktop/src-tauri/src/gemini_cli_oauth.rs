...
impl GeminiCliOauthRuntime {
    fn begin(
        ...
        if let Some(next) = inner.next_allowed.get(account_id).copied() {
            if now_ms < next {
                return Err(next);
            }
        }
        ...
        Ok(inner
            .projects
            .get(account_id)
            .filter(|cached| cached.credential_revision == revision)
...
mod tests {
    ...
    fn cadence_is_per_account_across_credential_revisions() {
        ...
        assert!(runtime.begin("account-a", "revision-two", NOW + 1).is_err());
    }
