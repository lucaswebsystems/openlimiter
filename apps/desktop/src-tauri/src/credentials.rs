...
/// API management keys are isolated from provider and account credentials.
/// Entry names are random UUIDs generated by the API spend subsystem.
pub const API_SPEND_CREDENTIAL_SERVICE: &str = "com.openlimiter.desktop.api-spend.v1";
...
/// A distinct keyring service for API spend management keys. Keeping this as
/// a separate Tauri state type makes it impossible to select the ordinary
/// credential namespace from an API spend command.
pub struct ApiSpendKeyringStore;

fn entry(service: &str, connection_id: &str) -> Result<keyring::Entry, CredentialError> {
    keyring::Entry::new(service, connection_id).map_err(|_| CredentialError::Store)
}
...
impl SecretStore for KeyringStore {
    fn store_secret(&self, connection_id: &str, secret: &str) -> Result<(), CredentialError> {
        entry(CREDENTIAL_SERVICE, connection_id)?
            .set_password(secret)
...
impl SecretStore for ApiSpendKeyringStore {
    fn store_secret(&self, entry_id: &str, secret: &str) -> Result<(), CredentialError> {
        entry(API_SPEND_CREDENTIAL_SERVICE, entry_id)?
            .set_password(secret)
            .map_err(keyring_error)
    }

    fn read_secret(&self, entry_id: &str) -> Result<Zeroizing<String>, CredentialError> {
        entry(API_SPEND_CREDENTIAL_SERVICE, entry_id)?
            .get_password()
            .map(Zeroizing::new)
            .map_err(keyring_error)
    ...
    fn delete_secret(&self, entry_id: &str) -> Result<(), CredentialError> {
        entry(API_SPEND_CREDENTIAL_SERVICE, entry_id)?
            .delete_credential()
            .map_err(keyring_error)
    }
...
mod tests {
    ...
    #[test]
    fn api_spend_uses_a_distinct_fixed_keyring_namespace() {
        assert_eq!(CREDENTIAL_SERVICE, "com.openlimiter.desktop");
        assert_eq!(
            API_SPEND_CREDENTIAL_SERVICE,
            "com.openlimiter.desktop.api-spend.v1"
        );
        assert_ne!(CREDENTIAL_SERVICE, API_SPEND_CREDENTIAL_SERVICE);
    }
