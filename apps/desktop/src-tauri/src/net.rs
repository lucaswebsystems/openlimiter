...
mod tests {
    ...
    async fn the_transport_double_sees_only_addresses_this_file_built() {
        ...
        let gemini_transport = RecordingTransport::replying(200, b"{}".to_vec(), None);
        fetch_gemini_cli_load(&gemini_transport, "gemini-secret")
            .await
        ...
        .await
        .expect("Gemini quota");
        recorded_urls.extend(gemini_transport.recorded_urls());
        assert_eq!(
            recorded_urls,
            vec![
    ...
    fn every_third_party_request_identifies_openlimiter() {
        ...
        for (auth, account) in [
            ...
            (AuthApplication::KimiSessionBearer, None),
