...
export function WindsurfMark({ className }: ToolMarkProps) {
  return <BrandGlyph path={WINDSURF_PATH} className={className} />;
}
...
export const plannedTools: readonly Tool[] = [
  ...
  { name: "Windsurf", Mark: WindsurfMark, state: "planned", detail: "Planned connector." },
...
/**
 * The row under the hero buttons: the five connectors that read something a
 * provider produced. Manual entry follows as a link, because it is a path
 * rather than a provider, so it is filtered out here rather than listed twice.
 */
export const heroMarks: readonly Tool[] = todayTools.filter(
  (tool) => tool.name !== "Manual entry",
);
