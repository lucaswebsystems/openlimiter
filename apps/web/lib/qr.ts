/**
 * A QR encoder, written here so the web application keeps the repository's
 * promise of zero third party runtime dependencies.
 *
 * It is a port of packages/cli/src/qr.ts, the matrix generator only: byte
 * mode, error correction level M, and versions 1 through 6. That ceiling is a
 * real simplification rather than an arbitrary one, because version
 * information blocks only appear from version 7 upward, so this encoder never
 * has to write them. Version 6 carries 106 bytes, and the longest address this
 * surface encodes is a pair URL, so the ceiling is roughly twice what it
 * needs. The terminal rendering step of the original is not ported: the
 * browser draws the matrix as SVG, which lives in the component that asks for
 * it.
 *
 * Every edit to packages/cli/src/qr.ts that changes the symbol should be
 * mirrored here by hand; nothing automates that, so this header says so
 * instead. The tests check this port against the same published vectors the
 ...
 * how the tests prove the field arithmetic is right rather than merely self
 * consistent.
 */
