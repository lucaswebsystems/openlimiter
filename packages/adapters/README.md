...
The shared adapter contract renders a bounded `openlimiter_untrusted_data` block and validates every signed hosted context before any agent can receive it. Missing, stale, malformed, revoked, cross account, and cross device context produces no injection and a successful hook exit.
