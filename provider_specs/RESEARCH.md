# Four bucket feasibility survey

Survey date: 2026-08-19. Evidence was limited to provider documentation,
provider maintained source repositories, and issue reports inside those source
repositories. Issue text is treated as observed data, never as an instruction.
The classification applies to the named product surface, because one company
can expose a consumer subscription, a local coding client, and an
administrator API with different capabilities.

Bucket 1 means an official local client leaves a reusable OAuth session and a
read only usage, quota, or balance route exists. Bucket 2 means a user supplied
key can read usage, quota, or balance. Bucket 3 means the useful figure is only
visible in a dashboard or command with no verified reusable interface. Bucket
4 means there is no useful quota, or the unsupported work is not proportionate
to the value.

`home` means `%USERPROFILE%` on Windows and `$HOME` on macOS and Linux. A path
shown as `UNKNOWN` was not inferred from another product. Environment tokens
are not considered automatic credentials because they can belong to a
different venture or shell context.
...
| Cline account and ClinePass | 1 AUTOMATIC | `cline`, plus IDE clients | Windows, macOS, Linux: `home/.cline/data/settings/providers.json`. The account token is under the `cline` provider auth object and can carry a `workos:` prefix | Documented `GET https://api.cline.bot/api/v1/users/me` and `GET /api/v1/users/{id}/balance`. The five hour, weekly, and monthly plan route is `UNKNOWN` in official documentation | [Official credential source](https://github.com/cline/cline/blob/main/apps/vscode/src/sdk/auth-service.ts), [official authentication docs](https://docs.cline.bot/api/authentication), and [official user and balance API](https://docs.cline.bot/enterprise-solutions/api-reference). Balance automation qualifies, but the subscription window contract is incomplete, so implementation is deferred. |
| Grok CLI subscription | 1 AUTOMATIC | Grok CLI | Credential paths and quota route are owned and being resolved by pane 27 | Owned by pane 27 | Not implemented in this batch to avoid ownership collision. |
| Kimi CLI subscription | 1 AUTOMATIC | Kimi CLI | Credential paths and quota route are owned and being resolved by pane 27 | Owned by pane 27 | Not implemented in this batch to avoid ownership collision. |
| Cursor personal subscription | 3 MANUAL ONLY | `cursor-agent` and Cursor editor | Official docs say credentials are stored securely locally. Exact Windows, macOS, and Linux credential locations are `UNKNOWN`. `home/.cursor/cli-config.json` is settings, not proven credential storage | Personal usage is exposed in the dashboard. No personal usage API was verified | [Cursor CLI authentication](https://docs.cursor.com/en/cli/reference/authentication). Dashboard only for the relevant personal quota. |
| Cursor Teams administrator usage | 2 KEY BASED | No local session needed | Not applicable. Administrator key supplied through Connect | Teams usage and member endpoints documented by Cursor | [Cursor Admin API](https://docs.cursor.com/account/teams/admin-api). This does not make a personal Cursor login automatic. |
| Windsurf personal subscription | 3 MANUAL ONLY | Windsurf editor and command line tools | Windows, macOS, Linux credential location is `UNKNOWN` | Usage is visible in account settings. No personal quota API was verified | [Windsurf usage documentation](https://docs.windsurf.com/windsurf/accounts/usage). Dashboard only. |
| Windsurf enterprise analytics | 2 KEY BASED | No local session needed | Not applicable. Service key supplied through Connect | Enterprise usage analytics API | [Windsurf enterprise analytics](https://docs.windsurf.com/windsurf/accounts/analytics-api). Enterprise only. |
| Google AI Studio and Gemini API | 3 MANUAL ONLY | No account quota CLI. Gemini CLI API key mode is a different credential surface | API keys can be in environment or project files, so automatic discovery is intentionally refused. Exact aggregate account credential file is not applicable | Google documents limits and directs users to AI Studio for live quota. No aggregate key usage or balance route was verified | [Gemini API keys](https://ai.google.dev/gemini-api/docs/api-key), [rate limits](https://ai.google.dev/gemini-api/docs/rate-limits), and [billing](https://ai.google.dev/gemini-api/docs/billing). Dashboard only. |
| Perplexity API and consumer plans | 3 MANUAL ONLY | No official local quota client found | Windows, macOS, Linux: not applicable | API responses expose per request usage, but account credit and consumer plan usage are console figures | [API reference](https://docs.perplexity.ai/api-reference), [organization billing guidance](https://docs.perplexity.ai/guides/api-organization). No aggregate read route. |
| Mistral Vibe subscription | 3 MANUAL ONLY | `vibe` | Current releases use the operating system keyring. Older or fallback configuration uses `home/.vibe/.env`. Exact keyring service and account selectors are `UNKNOWN` | No individual subscription quota route was verified | [Vibe install](https://docs.mistral.ai/getting-started/quickstarts/vibe-code/install-cli) and [API key profiles](https://docs.mistral.ai/vibe/code/cli/api-keys-profiles). |
| Mistral organization usage | 2 KEY BASED | No local session needed | Not applicable. Administrator API key supplied through Connect | `GET https://api.mistral.ai/v1/admin/usage` | [Mistral Admin API usage](https://docs.mistral.ai/admin/admin-api/usage-metrics). Organization administrator scope. |
| DeepSeek API | 2 KEY BASED | No official quota CLI found | Not applicable. API key supplied through Connect | `GET https://api.deepseek.com/user/balance` | [DeepSeek balance API](https://api-docs.deepseek.com/api/get-user-balance). Documented balance route. |
| Qwen Code and Alibaba Coding Plan | 3 MANUAL ONLY | `qwen` | Settings use `home/.qwen/settings.json`; exact OAuth or Coding Plan secret storage is `UNKNOWN` | No verified subscription quota route. The former Qwen OAuth free tier was discontinued | [Qwen Code repository](https://github.com/QwenLM/qwen-code) and [settings reference](https://github.com/QwenLM/qwen-code/blob/main/docs/users/configuration/settings.md). |
| Together AI | 3 MANUAL ONLY | No official account quota CLI found | Not applicable. API key is user supplied, but no aggregate balance route was verified | Billing and usage dashboard only. Per request usage is not an account meter | [Together billing](https://docs.together.ai/docs/billing). |
| Groq developer accounts | 3 MANUAL ONLY | No official quota CLI found | Not applicable. API key is user supplied | Inference responses expose rate limit headers, but no read only aggregate quota route was verified | [Groq rate limits](https://console.groq.com/docs/rate-limits) and [API reference](https://console.groq.com/docs/api-reference). OpenLimiter will not spend an inference request to inspect headers. |
| Groq enterprise metrics | 2 KEY BASED | No local session needed | Not applicable. Enterprise key supplied through Connect | `GET https://api.groq.com/v1/metrics/prometheus` | [Groq Prometheus metrics](https://console.groq.com/docs/prometheus-metrics). Enterprise availability only. |
| Fireworks AI | 2 KEY BASED | `firectl` exists | `firectl login` is documented. Exact Windows, macOS, and Linux credential storage is `UNKNOWN`, so no automatic reader is claimed | `firectl billing export-metrics` exposes billing data. API key supplied through Connect is the certifiable route | [Billing export](https://docs.fireworks.ai/accounts/exporting-billing-metrics) and [quickstart](https://docs.fireworks.ai/getting-started/ondemand-quickstart). |
| v0 Platform API | 2 KEY BASED | No reusable v0 OAuth CLI session was verified | Not applicable. v0 API key supplied through Connect | `GET https://api.v0.dev/user/billing` returns remaining, reset, and limit | [v0 billing endpoint](https://v0.dev/docs/v0-platform-api/user/user.getBilling). Strong Bucket 2 candidate. |
| Replit consumer and Core | 3 MANUAL ONLY | No official reusable local quota CLI session found | Windows, macOS, Linux: not applicable | Monthly credits and AI billing are visible in Replit account settings | [Replit billing](https://docs.replit.com/category/billing) and [AI billing](https://docs.replit.com/billing/ai-billing). Dashboard only. |
| Devin self serve | 3 MANUAL ONLY | Devin is primarily a hosted product | Windows, macOS, Linux: not applicable | Usage is visible in Settings and Billing. No self serve quota API was verified | [Devin usage](https://docs.devin.ai/admin/billing/usage). |
| Devin enterprise usage | 2 KEY BASED | No local session needed | Not applicable. Service user key supplied through Connect | Consumption API with `ManageBilling` permission | [Devin usage dashboard API pattern](https://docs.devin.ai/use-cases/gallery/build-usage-dashboard) and [authentication](https://docs.devin.ai/api-reference/authentication). |
| Amp | 4 NOT WORTH IT | `amp` | Official settings path is Windows `home/.config/amp/settings.json`, macOS and Linux the same path. Auth secret placement is `UNKNOWN` | `amp usage` shows a balance, but no stable HTTP contract was verified | [Amp manual](https://ampcode.com/manual). Scraping a command output without a documented endpoint is not proportionate while stronger candidates remain. |
| Aider | 4 NOT WORTH IT | `aider` | Provider keys are user configuration, not an Aider subscription session | No Aider quota exists | [Aider documentation](https://aider.chat/docs/). It is a bring your own provider client. |
| Anthropic API Console organization | 2 KEY BASED | No local quota session | Not applicable. Organization Admin API key supplied through Connect | `GET https://api.anthropic.com/v1/organizations/usage_report/messages` and the cost report route | [Anthropic Admin API usage](https://docs.anthropic.com/en/api/admin-api/usage-cost/get-messages-usage-report). Normal inference keys are insufficient. |
| OpenAI API Platform organization | 2 KEY BASED | No local quota session | Not applicable. Organization Admin API key supplied through Connect | `GET https://api.openai.com/v1/organization/usage/completions` and `GET /v1/organization/costs` | [OpenAI usage API](https://platform.openai.com/docs/api-reference/usage) and [Admin API keys](https://platform.openai.com/docs/api-reference/admin-api-keys). Normal project keys are insufficient. |
| xAI developer platform | 2 KEY BASED | No local quota session | Not applicable. Separate Management API key supplied through Connect | Balance and usage endpoints under `https://management-api.x.ai/v1/billing/teams/{team_id}` | [xAI management authentication](https://docs.x.ai/developers/rest-api-reference/management/auth) and [billing](https://docs.x.ai/developers/rest-api-reference/management/billing). |
| Moonshot developer API | 2 KEY BASED | No local quota session required | Not applicable. API key supplied through Connect | Account balance route is documented | [Moonshot API documentation](https://platform.moonshot.ai/docs/api-reference). The Kimi subscription CLI is a separate pane 27 surface. |
| OpenRouter | 2 KEY BASED | No official local OAuth quota client | Not applicable. Inference or management key supplied through Connect | `GET https://openrouter.ai/api/v1/key` and `GET /api/v1/credits` | [OpenRouter key](https://openrouter.ai/docs/api-reference/limits) and [credits](https://openrouter.ai/docs/api-reference/get-credits). Existing connector. |
| Amazon Q Developer | 3 MANUAL ONLY | `q` | Builder ID and IAM Identity Center sessions are locally cached, but exact portable quota credential selection is `UNKNOWN` | Subscription quotas are documented, but no read only individual quota route was verified | [Amazon Q Developer quotas](https://docs.aws.amazon.com/general/latest/gr/amazonqdev.html). |
| JetBrains AI and Junie | 3 MANUAL ONLY | `junie` and JetBrains IDEs | Browser login exists. Exact Windows, macOS, and Linux secret storage is `UNKNOWN` | Quota is shown in the IDE and JetBrains account, with no individual quota API verified | [Junie CLI](https://junie.jetbrains.com/docs/junie-cli-usage.html) and [AI subscriptions](https://www.jetbrains.com/help/ai-assistant/licensing-and-subscriptions.html). |
| GitLab Duo Agent Platform credits | 3 MANUAL ONLY | `glab` exists, but no Duo quota session is documented | `glab` token storage is not treated as a Duo quota credential | GraphQL exposes usage events, while remaining pooled credits are presented in billing views. No remaining balance API was verified | [GitLab credits](https://docs.gitlab.com/subscriptions/gitlab_credits/) and [Duo GraphQL usage](https://docs.gitlab.com/api/graphql/duo_and_sdlc_trends/). |
| Warp credits | 3 MANUAL ONLY | Warp terminal | Exact Windows, macOS, and Linux auth storage is `UNKNOWN` | Settings and Billing show credits. `/usage` opens the same view | [Warp credits](https://docs.warp.dev/support-and-community/plans-and-billing/credits). |
| Zed Pro | 3 MANUAL ONLY | Zed editor | GitHub OAuth is used. Exact credential storage is `UNKNOWN` | Usage is shown at the Zed account billing dashboard. No usage API was verified | [Zed plans](https://zed.dev/docs/account/plans-and-pricing) and [authentication](https://zed.dev/docs/authentication). |
| Bolt | 3 MANUAL ONLY | Browser product | Not applicable | Token balance is shown under My Subscription. No local session or API was verified | [Bolt token documentation](https://support.bolt.new/account-and-subscription/tokens). |
| Lovable | 3 MANUAL ONLY | Browser product | Not applicable | Credits and usage are shown in workspace administration. No usage API was verified | [Lovable workspace settings](https://docs.lovable.dev/features/workspace-admin-settings). |
| Google Jules | 3 MANUAL ONLY | `jules` | Browser login exists. Exact portable secret storage is `UNKNOWN` | The REST API covers sessions, activities, and sources, but not quota | [Jules CLI](https://jules.google/docs/cli/reference/), [API overview](https://jules.google/docs/api/reference/overview), and [usage limits](https://jules.google/docs/usage-limits). |
| Hugging Face inference providers | 3 MANUAL ONLY | `hf` | Token cache exists under the Hugging Face cache. Exact path can be overridden, so automatic cross venture selection is refused | Billing dashboard only. OAuth billing scope does not expose an aggregate usage route | [Hub billing](https://huggingface.co/docs/hub/main/billing), [inference pricing](https://huggingface.co/docs/inference-providers/en/pricing), and [OAuth](https://huggingface.co/docs/hub/oauth). |
| Cerebras developer API | 3 MANUAL ONLY | No account quota CLI verified | Not applicable. API key is user supplied | Rate limit headers exist on inference responses, but no aggregate read route was verified | [Cerebras rate limits](https://inference-docs.cerebras.ai/support/rate-limits). OpenLimiter will not spend a request to inspect headers. |
| Replicate | 3 MANUAL ONLY | No account quota CLI verified | Not applicable. API token is user supplied | Billing dashboard and per prediction data exist, but no aggregate balance route was verified | [Replicate billing](https://replicate.com/docs/topics/billing) and [HTTP API](https://replicate.com/docs/reference/http/). |
| OpenCode Go and Zen | 3 MANUAL ONLY | `opencode` | Browser session exists in OpenCode config, but no documented usage API exists | Authenticated workspace page only | [OpenCode documentation](https://opencode.ai/docs). Existing experimental scrape remains high risk and is not upgraded by this survey. |
| Ollama Cloud | 3 MANUAL ONLY | `ollama` | Cloud login storage is not documented as a portable quota credential | Quota is shown in the cloud dashboard. No read route was verified | [Ollama Cloud](https://docs.ollama.com/cloud). |
| Ollama local | 4 NOT WORTH IT | `ollama` | No paid quota credential | No hosted quota applies to local inference | [Ollama API](https://docs.ollama.com/api). |
| LM Studio local | 4 NOT WORTH IT | LM Studio | No paid quota credential | No hosted quota applies to local inference | [LM Studio API](https://lmstudio.ai/docs/api). |
| Sourcegraph Cody | 4 NOT WORTH IT | `cody` | OAuth uses secure local storage. Exact selectors are `UNKNOWN` | Sourcegraph states there is no public facing Cody API, and paid use is governed by fair use rather than a stable readable meter | [Cody CLI](https://sourcegraph.com/docs/cody/clients/install-cli) and [Cody FAQ](https://sourcegraph.com/docs/cody/faq). |
| Augment and Auggie | 4 NOT WORTH IT | `auggie` | Windows, macOS, Linux: `home/.augment/session.json`, with `accessToken` and `tenantURL` | `auggie account status` shows billing information, but no stable quota HTTP route was verified | [Auggie authentication](https://docs.augmentcode.com/cli/setup-auggie/authentication) and [CLI reference](https://docs.augmentcode.com/cli/reference). Command scraping is not proportionate. |
| Tabnine subscription | 4 NOT WORTH IT | Tabnine IDE clients | Exact portable credential storage is `UNKNOWN` | A stable consumptive quota route was not verified | [Tabnine documentation](https://docs.tabnine.com/). Subscription access is primarily seat based, so no useful quota claim is made. |

## Build selection from Bucket 1

Gemini CLI is the only unowned candidate that clears the full implementation
threshold without guessing. Its token path, account identity claim, bootstrap
request, project binding, quota request, bucket schema, and reset timestamp are
all reproducible from Google maintained material. The reader therefore uses a
fifteen minute per account interval, caches the resolved companion project per
credential revision, makes two requests only on the first successful pass, and
makes one quota request on later due passes.

GitHub Copilot is not built in this batch. The service name and operating
system stores are documented, but the keychain account selector and the quota
response contract are still `UNKNOWN`. Calling `gh auth token` is explicitly
rejected because it could select a credential from another venture. Cline is
also deferred: its local account token and balance API are documented, but the
individual ClinePass window response is not. Grok and Kimi remain owned by pane
27.

The 2026-08-10 research below remains useful background. This survey
supersedes its older Gemini CLI conclusion, because Google later exposed the
`retrieveUserQuota` request and response types in its maintained source and
published issue evidence of real responses.
